...
(function ($) {
    $.compileTemplate = function (template, begin, end) {
        ...
        var code = "self = self || {}; with ($.wayneeseguinTemplates.helpers) { with (self) {" +
            "var _result = '';" +
            template
                .replace(/[\t\r\n]/g, ' ')
                .replace(/^(.*)$/, end + '$1' + begin)
    ...
    $(function() {
        $("script[type='text/x-jquery-template']").each(function() {
            $.wayneeseguinTemplates[this.title] = $.compileTemplate(this.innerHTML, "<%", "%>");
            $.wayneeseguinTemplates[this.title].templateName = this.title;
        });
    });
    ...
    $.wayneeseguinTemplates = {
        helpers: {
            partial: function(name, json) {
                return $.wayneeseguinTemplates[name](json || {}, true);
            }
        }
    }

    $.loadTemplates = function() {
        $.wayneeseguinTemplates = $.wayneeseguinTemplates || {};
        $("script[type='text/x-jquery-template']").each(function() {
            $.wayneeseguinTemplates[this.title] = $.compileTemplate(this.innerHTML, "<%", "%>");
        });
    }
